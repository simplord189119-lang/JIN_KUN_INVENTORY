...
import { CharactersProvider, useCharacters } from "./CharactersContext.jsx";
import CharacterFormModal from "./CharacterFormModal.jsx";
import SyncConfirmModal from "./SyncConfirmModal.jsx";
...
/* ============================================================================
   DESIGN TOKENS — moved to theme.js (C / ELEMENTS / WEAPON_TYPES), imported
   at the top of this file, so CharacterFormModal/SyncConfirmModal/
   CharactersContext can share them without a circular import.
============================================================================ */
...
function AppInner() {
  ...
  // --- dynamic character roster (Firestore-backed, real-time) ---
  const { characters: dynamicCharacters, isAdmin, syncBusy: rosterSyncBusy } = useCharacters();
  const [addCharOpen, setAddCharOpen] = useState(false);
  const [editCharItem, setEditCharItem] = useState(null); // character object being edited, or null
  ...
  const priorityChars = useMemo(
    () => dynamicCharacters.filter(c => characterPriorities[c.id] === dashPriorityTier),
    [dynamicCharacters, characterPriorities, dashPriorityTier]
  );
  ...
  const ownedCharList = useMemo(
    () => dynamicCharacters.filter(c => ownedCharacters[c.id]),
    [dynamicCharacters, ownedCharacters]
  );
  ...
  const filteredChars = useMemo(
    ...
    [dynamicCharacters, charSearch, charElementFilter, charWeaponFilter]
  );
      ...
      <main className="max-w-5xl mx-auto px-4 py-5 pb-28">
        ...
        {activeTab === "resonators" && (
          <RosterGrid
            ...
            onToggleOwned={toggleOwned}
            isAdmin={isAdmin}
            onAddNew={() => setAddCharOpen(true)}
            onEditItem={(item) => setEditCharItem(item)}
            onCheckUpdates={() => setSyncModalOpen(true)}
            syncBusy={rosterSyncBusy}
          />
        )}

        {addCharOpen && <CharacterFormModal onClose={() => setAddCharOpen(false)} />}
        {editCharItem && <CharacterFormModal character={editCharItem} onClose={() => setEditCharItem(null)} />}
        {syncModalOpen && <SyncConfirmModal onClose={() => setSyncModalOpen(false)} />}
...
/* ============================================================================
   ROOT EXPORT — wraps AppInner in CharactersProvider so useCharacters() is
   available throughout (AppInner, RosterGrid's admin toolbar, the modals).
============================================================================ */
...
function RosterGrid({ title, items, search, setSearch, priorities, setTier, kind, elementFilter, setElementFilter, weaponFilter, setWeaponFilter, owned, onToggleOwned, isAdmin, onAddNew, onEditItem, onCheckUpdates, syncBusy }) {
      ...
      {kind === "character" && isAdmin && (
        <div className="flex gap-2 mb-3">
          <button
            ...
            style={{ background: C.gold, color: C.void }}
          >
            <Plus size={14} /> Add Resonator
          </button>
          <button
            onClick={onCheckUpdates}
            disabled={syncBusy}
            className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg text-xs font-bold disabled:opacity-60"
            style={{ border: `1px solid ${C.starlight}55`, color: C.starlight }}
          >
            {syncBusy ? <Loader2 size={13} className="animate-spin" /> : <Search size={13} />} Check for Updates
          </button>
        </div>
      )}
            ...
            >
              ...
              {kind === "character" && isAdmin && (
                <button
                  ...
                  style={{ background: `${C.void}AA`, border: `1px solid ${C.border}` }}
                  title="Edit"
                >
                  <Pencil size={12} color={C.ivoryDim} />
                </button>
              )}
              {isOwned && (
                <span
